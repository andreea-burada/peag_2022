...
#CROSSOVER for binary or integer reprezentation
#
# one-point crossover between x and y, array with n components
# I :x,y,n as above
#E: c1,c2 - two children with n components
#
...
def crossover_unipunct(x,y,n):
    #randomly generates the crossover point, between 1 and n-1 to take effect
    # in position n is quality
    #
...
# uniform crossover between x and y, array with n components
# I :x,y,n as above
# O: c1,c2 - two children
#
...
# CROSSOVER for permutation reprezentation - a permutation of dim n is with the elements 0,1, ..., n-1
#PMX operator
# I :x,y permutation with n dimension
# O: c1,c2 - two resulted children
#
...
def PMX(x,y,n,p1,p2):
    #child initialization - a vector with all elements -1 - values that s = not to be in 0, ..., n-1
    #
    #initializare copil - un vector cu toate elementele -1 - valori care s = sa nu fie in 0,...,n-1
    c=-np.ones(n,dtype=int)
    #copy the common sequence in the child c
    #
    #copiaza secventa comuna in copilul c
    c[p1:p2+1]=x[p1:p2+1]
    # common sequence analysis - in permutation y
    #
    ...
    for i in range(p1,p2+1):
        ...
        if a not in c:
            ...
            while not plasat :
                b=x[curent]
                # poz = position in which b is in y
                #
...
#OCX operator
#I: permutations x, y of dimension n
#O: the resulting children c1,c2
#
#operatorul OCX
#I: permutarile x,y de dimensiune n
#E: copiii rezultati c1,c2
...
#apply OCX to x, y of size n, with recombination sequence (p1, p2)
#
#aplica OCX pe x,y de dimensiune n, cu secventa de recombinare (p1,p2)
def OCX(x,y,n,p1,p2):
    # copy common sequence in c2
    #
    #copiaza secventa comuna in c2
    c2=[x[i] for i in range(p1,p2+1)]
    # calculates z based on y: components starting with p2 to n and then from 0 to p2-1, excluding items that have already been copied
    #
...
# CROSSOVER for the real-valued representation
# crossover x with y - single arithmetic, vectors with n components of the ndarray type
# I :x,y,n as above
#    alpha - weight
#O: c1,c2 - two children - without evaluation
#
...
def crossover_singular (x, y, n,alpha):
    #randomly generates the gene in which the recombination/crossover is made
    #
